...
Replace every `caches.X(...)` read site in the service with the provider. Done: `access/access.go`, `api/handlers/guilds/handler.go`, `actions/parser/permissions.go`. Left: `manager/webhook/message.go` (6), `actions/template/data.go` (3), `api/handlers/custom_bots/handler.go` (2), `command/cmd_message.go` (1), `api/handlers/send_message/handler.go` (1). Every one of them currently treats a cache miss as an error or as "no permissions"; with the provider a miss is a real REST error or 404 and should be handled as such. Remove `cache.Caches` from every constructor that only used it for these reads.
