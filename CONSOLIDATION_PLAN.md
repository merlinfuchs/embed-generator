...
Replace every `caches.X(...)` read site in the service with the provider. All done. `cache.Caches` is gone from every constructor, from `api.Env` and from `EventHandler`; only `embedg/embedg.go` still builds the compat caches, and B7 deletes those with the rest of Stateway. Every one of them currently treats a cache miss as an error or as "no permissions"; with the provider a miss is a real REST error or 404 and should be handled as such. Remove `cache.Caches` from every constructor that only used it for these reads.
